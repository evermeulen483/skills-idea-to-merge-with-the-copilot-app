...
- Keep browser-only code behind a `client:load` boundary and access `localStorage` only after client-side hydration, never at module scope or during server rendering, so SSR and static builds never touch `localStorage`.
